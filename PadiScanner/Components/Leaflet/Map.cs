...
namespace PadiScanner.Components.Leaflet
{
    ...
    public class Map : InteropObject
    {
        ...
        public async Task<Map> FitBounds(LatLngBounds bounds)
        {
            if (bounds.JSBinder is null)
            {
                await bounds.BindJsObjectReference(this.JSBinder);
            ...
            bounds.GuardAgainstNullBinding("Cannot set map view. No JavaScript binding has been set up for the center argument.");
            var module = await JSBinder.GetLeafletMapModule();
            await module.InvokeVoidAsync("LeafletMap.Map.fitBounds", this.JSObjectReference, bounds.JSObjectReference);
            return this;
        }
